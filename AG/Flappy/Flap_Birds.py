...
class FlapBirds:
    def __init__(self, window_size):
        ...
        # Tenta carregar modelos treinados
        modelo_path = os.path.join(os.path.dirname(__file__), '..', 'modelos_treinados.pkl')
        if os.path.exists(modelo_path):
            ...
            with open(modelo_path, 'rb') as f:
                ...
                if isinstance(dados_carregados, dict):
                    cerebros_iniciais = dados_carregados["cerebros"]
                else:
                    # Formato antigo (lista)
                    cerebros_iniciais = dados_carregados
                    
            print(f"  {len(cerebros_iniciais)} modelos carregados!")
        else:
            print("⚠ Nenhum modelo treinado encontrado. Usando AG padrão...")
            self.ag = AlgG(tamanho_populacao=100, taxa_mutacao=0.1)
            cerebros_iniciais = self.ag.create_populacao(4, 6, 1)
        
        self.birds = [Bird(cerebro) for cerebro in cerebros_iniciais]
        self.birds_mortos = [] # Lista para os que baterem
    ...
    def board(self):
        ...
        for bird in self.birds:
                    if bird.vivo:
                        self.window.blit(self.bird, (bird.x, bird.y))
        ...
        if self.in_play:
            ...
            for bird in self.birds:
                if bird.vivo:
                    ...
                    if distancia_x < -130: # Se o cano 1 já ficou para trás, ele deve olhar para o cano
                        distancia_x = self.pipe_2_pos[0] - bird.x
                        distancia_yB = self.pipe_2_pos[1] - 400 # Posição do buraco do cano
                    else:
                    ...
                    inputs = [
                        bird.y / 720.0,               # Divide pela altura da tela
                        bird.v_vertical / 15.0,       # Divide por uma velocidade alta
                        distancia_x / 1280.0,         # Divide pela largura da tela
                        diferenca_y / 720.0
                    ]
    ...
    def scoreboard(self):
        if self.pipe_1_pos[0] < 100 + 51 and self.pipe_1_pos[0] + 123 > 100:
        ...
        for bird in self.birds:
            if bird.vivo:
                ...
                if self.pipe_1_pos[0] < bird.x + 51 and self.pipe_1_pos[0] + 123 > bird.x:
                    # Verifica colisão com cano: topo OU fundo
    ...
    def evolucao(self):
        print(f"Geração {self.geracao_at} morreu. A iniciar Evolução...")
        
        # Inicializa AG se não foi feito
        if not hasattr(self, 'ag'):
            self.ag = AlgG(tamanho_populacao=len(self.birds_mortos), taxa_mutacao=0.1)
        
        # Extrair os cérebros antigos e as seus fitness
        populacao_antiga = [b.cerebro for b in self.birds_mortos]
        fitnesses = [b.fitness for b in self.birds_mortos]
        
        #nova geração
        nova_populacao_cerebros = self.ag.create_geracao(populacao_antiga, fitnesses)
        
        # novos cérebros em novos pássaros 
        self.birds = [Bird(cerebro) for cerebro in nova_populacao_cerebros]
        self.birds_mortos = []
        
        self.geracao_at += 1
        
        #Reset posições dos canos
        self.score = 0
        self.bird_passing_through_obstacle = False
        self.pipe_1_pos = [ 400, self.new_height_for_pipe()]
        self.pipe_2_pos = [ 800, self.new_height_for_pipe()]
